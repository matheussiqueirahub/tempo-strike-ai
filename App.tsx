...
const App: React.FC = () => {
  ...
  // Animation state for multiplier pop
  ...
  // Game Logic Handlers
  const handleNoteHit = useCallback((note: NoteData, accuracy: HitAccuracy) => {
     let points = 0;
     ...
     setCombo(c => {
       ...
       setMultiplier(prevMult => {
           if (newMult !== prevMult && newMult > prevMult) {
               // Trigger animation on multiplier increase
               setMultiplierScale(2.0);
               setTimeout(() => setMultiplierScale(1), 300);
           }
           return newMult;
       });
     ...
     // Use strict local multiplier or assume state update is fast enough for next frame? 
     // For safety, we use the value we just calculated or rely on effect. 
     // Simplest approximation for current frame: use current state multiplier if it hasn't updated yet, 
     // but since this runs once, let's just use current 'multiplier' state which lags by one hit for the *very first* threshold cross. 
     // That's acceptable for arcade feel.
     setScore(s => s + (points * multiplier));
     
     // Health logic based on accuracy
     const healthGain = accuracy === HitAccuracy.PERFECT ? 4 : accuracy === HitAccuracy.GOOD ? 2 : 1;
  ...
  const startGame = async () => {
    ...
    DEMO_CHART.forEach(n => { n.hit = false; n.missed = false; n.accuracy = undefined; });
          ...
          <div className="flex justify-between items-start text-white w-full">
             ...
             <div className="text-center">
                 ...
                 <div className="mt-2 flex flex-col items-center">
                     ...
                     {multiplier > 1 && (
                         <div 
                             className="mt-2 transition-transform duration-300 ease-out origin-center"
                             style={{ transform: `scale(${multiplierScale})` }}
                         >
                             <div className={`
                                 px-4 py-1 rounded-full border-2 shadow-[0_0_20px_currentColor]
                                 font-black italic tracking-widest text-lg flex items-center gap-2
              ...
              {gameStatus === GameStatus.IDLE && (
                  <div className="bg-black/80 p-12 rounded-3xl text-center border-2 border-blue-500/30 backdrop-blur-xl max-w-lg">
                      ...
                      <div className="text-white/30 text-sm text-center mt-8">
                           Created by <span className="text-blue-400 font-semibold">Matheus Siqueira</span>
                      </div>
                  </div>
              )}
